...
void StakeMiner(CWallet *pwallet)
{
    ...
    RenameThread("Czecoin-stakeminer");
...
void static CzecoinMiner(CWallet *pwallet)
{
    printf("CzecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    ...
    try { loop {
        ...
        {
            printf("CzecoinMiner : no more PoW blocks\n");
            return;
        }

        IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

        printf("Running CzecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CzecoinMiner terminated\n");
        throw;
    }
}

void GenerateCzecoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CzecoinMiner, pwallet));
}
